...
- Toggle: `VITE_USE_MOCKS` in `.env.local` (copy from `.env.example`).
  ...
  handlers.
- Handlers live in `src/mocks/handlers.ts`. **Currently empty** — see
  `TODO.md`: writing them needs SDS §6 (exact response shapes) and the
  Ticket #3 client modules, both still blocked.
