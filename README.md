...
- `/login` — guest-only (redirects to `/dashboard` if already authenticated)
...
- Handlers live in `src/mocks/handlers.ts`. **Auth handlers only so far**
  (`csrf`, `login`, `me`, `logout`, `forgot-password`, `reset-password`)
  — see `TODO.md`: the rest need SDS §6 (exact response shapes) and §2,
  both still blocked. The auth ones are provisional too (see below),
  in-memory, and reset on page reload.
...
- "Sign up" links to `/register`, which doesn't exist as a route yet
  (Ticket #10).
