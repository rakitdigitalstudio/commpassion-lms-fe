...
- Handlers live in `src/mocks/handlers.ts`. **Auth handlers only so far**
  (`csrf`, `login`, `me`, `logout`) — see `TODO.md`: the rest need SDS §6
  (exact response shapes) and §2, both still blocked. The auth ones are
  provisional too (see below), in-memory, and reset on page reload.
...
matching the mockup. Submits through `useAuth().login()`; a 401
(`InvalidCredentialsError`) shows a generic "Invalid email or password"
message, any other failure shows a generic error, and success navigates
to `/dashboard`.

- **`VITE_IS_COMING_SOON`** (`.env.example`): `"true"` hides the sign-in
  form and sign-up link, showing a "coming soon" notice instead.
...
- "Forgot Password?" and "Sign up" link to `/forgot-password` and
  `/register`, which don't exist as routes yet (Tickets #10/#11).
